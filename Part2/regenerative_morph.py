    ...
    for j in range(w - 1, height - w):
        for i in range(w - 1, width - w):
            ...
            for nnp_idx in nnp_idxs:
                ...
                ssp1_hat = pixel_complete(pixelpos, nnp_idx, source1_complete_dict, target1_complete_dict,
                                          source1_patches, nnp_idx, w, N_T)
                sp1_hat += ssp1_hat
                if np.sum(ssp1_hat) != 0.0:
                    n1 += 1

                    # Corresponding patch in source2 for complete
                ssp2_hat = pixel_complete(pixelpos, nnp_idx, source2_complete_dict, target2_complete_dict,
                                          source2_patches, nnp_idx, w, N_T)
                sp2_hat += ssp2_hat
...
source1 = cv2.imread('frame50.png')[..., ::-1]
source2 = cv2.imread('frame60.png')[..., ::-1]
ref = cv2.imread('frame55.png')[..., ::-1]
ws = [3, 5, 7]
